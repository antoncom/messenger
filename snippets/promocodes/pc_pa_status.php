		...
		if ($extended['promo_actions'] !== NULL) {
			if (array_search($pa_id, $extended['promo_actions']) !== FALSE) {
				...
				if (count($res) > 0) {
					...
					$output = '<a data-pa_id="'.$pa_id.'" href="javascript:;" class="pc_status '.$colored.'" data-html="true" data-toggle="popover" data-placement="left" data-animation="false" title="Промо-код активен" data-content="'.$stat . $doings .'">'.$res[0]['pagetitle'].'</a>';
				}
				else{
					...
					$colored = 'expired';
					$output = '<a data-pa_id="'.$pa_id.'" href="javascript:;" class="pc_status '.$colored.'" data-html="true" data-toggle="popover" data-placement="left" data-animation="false" title="Акция подключена" data-content="' . $doings .'">Извлечь</a>';
				}
			}
			else{
				// Подключение к акции
...
//				$output = "<a href='javascript:;' class='pc_status notincluded' data-html='true' data-toggle='popover' data-placement='left' data-animation='false' title='Участие в промо-акции' data-content='".$join_msg."'></a>";
				$output = '<a data-pa_id="'.$pa_id.'" href="javascript:;" class="pc_status notincluded" data-html="true" data-toggle="popover" data-placement="left" data-animation="false" title="Участие в промо-акции" data-content="'.$join_msg.'">&nbsp;</a>';
			}
		}
		return $output;
	}
